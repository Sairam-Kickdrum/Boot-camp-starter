...
whose `implements:` frontmatter names an `FR-*`. It doesn't live inside either tree: a
feature-request doesn't know whether it's been built, and a FEAT doesn't know what intent it
satisfies, without something that reads both sides and judges the match.
...
- **Never infer a match from naming alone** (e.g. a FEAT and FR sharing a similar title) — the
  ...
  behavior, every time.
- This is advisory, not a gate — the caller (`/init-code-wiki` Step 5c) always writes the link and
  the verdict into `Feat-NNNN-{feature-id}/Index.md` regardless of outcome; it never withholds the
  link because the verdict came back `partial` or `unmatched`. Silence would hide the exact signal
  a human needs. (There is no decisions ledger in this wiki — a verdict is never written to a
  `DEC-*` record; the FR side of the link is verifiable directly from
  `feature-requests/{feature-id}/feature-request.md` if a human wants to read the intent it's
  checked against.)
