...
3. For each item, extract:
   ...
   - `evidence_quote` — the verbatim line(s) grounding it, quoted exactly from the
     `[EVIDENCE]`-wrapped text (never paraphrased — this is what `wiki-writer.md` later cites
     inline on the fact bullet it writes).
...
as `not_relevant` for reporting purposes — no fact written, no ticket.

## Rules

- `evidence_quote` must be copied verbatim from the sanitized text — never paraphrase it. The
  whole point of citing evidence inline on a fact bullet is to ground it in what was actually
  said, not a restatement.
- Don't invent decisions that weren't discussed — if a meeting was inconclusive on a topic, that's
  an `open_question` classification later (via `wiki-fact-classifier.md`), not a fabricated
  settled fact.
- One discussion item per candidate — don't bundle "we decided X and also Y" into a single item
  if X and Y are separable decisions; `wiki-feature-mapper.md` and `wiki-relationship-scanner.md`
