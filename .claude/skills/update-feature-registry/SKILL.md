...
description: SUPERSEDED — docs/features/ is no longer the store; the code wiki is. The code-wiki equivalent is the closeout stage's closeout-feat-recorder sortie (AI-77/FD-711), run per-ticket from apps/server/src/activities/closeout.ts, not a skill invoked by hand. Do not run. Update docs/features/ after a ticket is completed. Updates or creates the relevant per-feature index.md files and keeps the master index in sync so future planning can identify what might break.
...
>
> `docs/features/` is no longer the store. The **code wiki**
> (`code-wiki/{project-id}/`) is the single source of truth about the codebase, and
> **`/init-code-wiki` now performs the full codebase scan this skill used to** — the same
> passes, dispatched to parallel exploration subagents, written into the code wiki's own
> shape (v4-DEC-021: "codebase-wiki is the feature registry, extended in place — not a new
> store").
>
> Running this skill re-creates the parallel store that decision exists to prevent, and
> nothing reads `docs/features/` any more.
>
> This file is kept, unrun, only so the fold is revertible in one step until it has run
> against a real repository. Delete it once it has. Everything below is the historical
> instruction set and describes a path that is no longer wired to anything.
...
```bash
grep -r "{endpoint-path}" . --include="*.ts" --include="*.tsx" -l
```

For each modified shared utility, search for importers:
```bash
grep -r "{utility-name}" . --include="*.ts" -l
```

> `.` and `.` are the paths configured in your project's CLAUDE.md (e.g., `frontend/`, `backend/services/`). Adapt the glob patterns to match your project's actual directory structure.
