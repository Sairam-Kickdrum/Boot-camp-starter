...
`disable-model-invocation: true` above means this skill can only be reached via a user-typed
`/implement <ticket-id>` — never via the Skill tool or a nested call from another skill's
reasoning. This is intentional: it stops ticket content or ambient model reasoning from
self-triggering a full implementation run through some undocumented path. The one documented
exception is `linear-implement-trigger/SKILL.md`, which — only after a human has explicitly set
`IMPLEMENT_POLL_ENABLED=true` and registered it on a schedule — follows these instructions
directly inline (the same "follow the skill's instructions" composition pattern used everywhere
else in this codebase), rather than invoking this skill as a nested call. No other skill should
do the same without an equivalent explicit, human-gated opt-in.
...
| `UPDATE_FEATURE_REGISTRY` | init-code-wiki (refresh mode) | Yes — flows into code wiki |
| `UPDATE_CODE_WIKI` | update-code-wiki | Terminal — done |
...
**Goal**: Bring the code wiki current, so it reflects the whole codebase rather than just this
ticket's diff.

1. Follow the `init-code-wiki` skill instructions **in refresh mode** — say so explicitly when
   invoking it, since its default mode stops at a one-time guard when the tree already exists.
   A refresh re-runs the full scan (its Steps 5a–5c) and rewrites content over the existing
   tree; it never re-scaffolds, never renumbers a `Feat-NNNN`, and never deletes a feature page.

   A **full rescan every ticket**, not an incremental diff-based update: a diff-based update only
   corrects the pages a ticket touched, while the couplings that actually break are the ones in
   pages it did not touch.

   Both `/update-feature-registry` and `/init-feature-registry` are retired — the first has no
   successor, the second was folded into `/init-code-wiki`. Do not invoke either here or anywhere
   else in `/implement`.

2. Update `**Phase**` to `UPDATE_CODE_WIKI`. **Auto-continue**.

---

### UPDATE_CODE_WIKI Phase

**Goal**: Historical pass-through — no longer does anything.

1. The previous phase's refresh run already rewrote every code-wiki file
   (`Architecture/Overview.md`, `Schemas/schemas.md`, `Features/Feat-NNNN-*/Index.md`,
   `Features/index.md`), so there is nothing left for this phase to do. It's kept as a phase
   name only so `execution-state.md` files already mid-flight from before this change still
   resolve to a known phase; do not follow `update-code-wiki`'s own instructions here — that
   skill expects `feature-registry-manifest.json`, which nothing writes. Ticket-by-ticket
   reality upkeep now happens post-merge, in closeout's closeout-feat-recorder sortie
   (AI-77/FD-711) — not in this phase and not by invoking a skill named `update-code-wiki`,
   which does not exist.

2. Update `**Phase**` to `DONE` and `**Status**` to `COMPLETE`.

3. Tell the user workflow is complete. List all PR URLs and any code-wiki files updated.
