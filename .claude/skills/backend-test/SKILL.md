...
6. Run tests with the commands below — the `apps/api` workspace uses Vitest, not a JUnit-style runner, so "class/method" selection below means "test file / test name".

## Running

This repo's backend is `apps/api` (Fastify + TypeScript, Vitest). Tests are colocated `*.test.ts`
files (e.g. `apps/api/src/services/booking-service.test.ts`, `apps/api/src/errors/app-error.test.ts`),
...
# Single test file
npm test --workspace=apps/api -- src/services/booking-service.test.ts

# Watch mode
npm run test:watch --workspace=apps/api

# Filter by test name (Vitest -t)
npm test --workspace=apps/api -- -t "should cancel order"
```

Note: `apps/api`'s Vitest run currently requires **Node 24** (`nvm use 24`) due to a Fastify 5 /
