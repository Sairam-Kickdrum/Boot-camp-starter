...
product-wiki content — Requirements, Business Rules, Key Facts — arrives organically, ticket by
ticket, through `product-align-loop`'s own wiki-PR mechanism. There is no decisions ledger in this
wiki — a fact lands directly in the relevant `feature-request.md`, cited with its evidence inline.
...
in the generated content. **Flag the inference, not the evidence** — the flag says the fact is
unverified; it is not licence to name the directory or file it came from. Name the capability a
directory *serves*, in the product's own vocabulary, and if a directory's purpose cannot be stated
that way from the evidence read, it is not yet a feature area — leave it out and say so in the
Report rather than seeding a stub named after a folder.
...
`## Evidence` and `## Relationships` are unaffected — a feature-request id or a code-wiki link is a
reference, not a code identifier.
...
⚠ **This skill is the likeliest place for that rule to be broken, because its input is the codebase
itself.** Step 4b derives candidate feature areas from *directory names* (`services/*`, `apps/*`,
`packages/*`, `src/modules/*`), and Step 4a's evidence files are source. Translating that evidence
into product language is the work, not an optional polish pass — a stub whose title is a package name
and whose `## Current State` quotes a file path is precisely the output that made this rule
necessary.
...
title: "{Feature Name}"
slug: {feature-id}
feature_id: FR-NNNN   # scan existing FR frontmatter for the current max and increment
description: "{One canonical sentence — what this feature request is, in plain language}"
domain: {business-domain, best guess from the README/scan}
aliases: []
owners: []
status: active
last_updated: {date}
...
For a not-yet-built feature area, omit the `## Evidence` heading entirely (there is no Feat page to
point to) and say so plainly: `No codebase-wiki page exists — this feature isn't built yet.`

One deliberate, narrow deviation from `wiki/SCHEMA.md`'s letter, intentional, not an oversight:
`## Current State` gets real evidence-linked content (everything else stays literal `Nothing
recorded yet.`) because a README/scan legitimately establishes observable reality, while
`Requirements`/`Business Rules` are product *intent* this skill has no license to invent.
`dependencies`/`conflicts` are left empty at stub time — `wiki-relationship-scanner.md` populates
them later, once real facts exist to compare across features; a coarse stub has nothing yet worth
cross-checking.

Then update `wiki/{project-id}/feature-requests/index.md` with one new row per newly-created
stub, including the `## Dependency Graph` section if this run set any `dependencies`/`conflicts`
(it won't, at stub time, per the above — noted here for completeness). There is no `decisions/`
tree in this wiki — nothing to touch there.
...
- Never fabricate a Business Rule, Requirement, or Key Fact — `(unknown — needs manual fill-in)` /
